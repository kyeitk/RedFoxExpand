...
public final class SchemaV2Parser {
    private static final Set<String> ROOT_FIELDS = Set.of("api_version", "definitions");
    ...
    private MatchSpec parseMatch(JsonObject json, String label) {
        ...
        return switch (key) {
            case "all" -> new MatchSpec.All(parseTerms(value, label + ".all"));
            case "any" -> new MatchSpec.Any(parseTerms(value, label + ".any"));
            case "not" -> new MatchSpec.Not(parseMatch(object(value, label + ".not"), label + ".not"));
            case "exact_screen_class" -> new MatchSpec.ExactScreenClass(fullClass(value, label));
            case "assignable_screen_class" -> new MatchSpec.AssignableScreenClass(fullClass(value, label));
            case "exact_screen_simple_class" -> new MatchSpec.ExactScreenSimpleClass(simpleClass(value, label));
            case "assignable_screen_simple_class" -> new MatchSpec.AssignableScreenSimpleClass(simpleClass(value, label));
            case "exact_menu_class" -> new MatchSpec.ExactMenuClass(fullClass(value, label));
            case "assignable_menu_class" -> new MatchSpec.AssignableMenuClass(fullClass(value, label));
            case "exact_menu_simple_class" -> new MatchSpec.ExactMenuSimpleClass(simpleClass(value, label));
            case "assignable_menu_simple_class" -> new MatchSpec.AssignableMenuSimpleClass(simpleClass(value, label));
            case "screen_title_key" -> new MatchSpec.ScreenTitleKey(nonEmpty(value, label));
            case "screen_title_text" -> new MatchSpec.ScreenTitleText(nonEmpty(value, label));
            case "menu_type" -> new MatchSpec.MenuType(identifier(value, label));
            case "resource_location" -> new MatchSpec.ResourceLocation(identifier(value, label));
            case "mod_namespace" -> new MatchSpec.ModNamespace(namespace(value, label));
            default -> throw failure(label, "unknown matcher operator: " + key);
        };
    }

    private List<MatchSpec> parseTerms(JsonElement element, String label) {
        JsonArray array = array(element, label);
        if (array.isEmpty() || array.size() > ResourceLimits.MAX_DEFINITIONS_PER_FILE) {
    ...
    private GuiDefinition.Geometry parseGeometry(JsonObject json, String label) {
        rejectUnknown(json, Set.of("x_offset", "y_offset", "width_offset", "height_offset"), label);
    ...
    private List<GuiDefinition.SlotRule> parseSlotRules(JsonObject root, String label) {
        ...
        Set<String> fields = Set.of("slots", "target_x", "target_y", "target_class_name",
    ...
    private List<GuiDefinition.Sprite> parseSprites(JsonObject root, String label) {
        ...
        Set<String> fields = Set.of("texture", "animation", "x", "y", "z", "u", "v",
    ...
    private GuiDefinition.Animation parseAnimation(JsonObject json, GuiDefinition.TextureSpec fallback, String label) {
        rejectUnknown(json, Set.of("frames", "frame_duration_ms", "loop", "condition",
        ...
        if (frames.isEmpty() || frames.size() > ResourceLimits.MAX_ANIMATION_FRAMES) {
        ...
        for (int index = 0; index < frames.size(); index++) {
            ...
            rejectUnknown(frame, Set.of("texture", "duration_ms"), frameLabel);
    ...
    private GuiDefinition.TextureSpec parseTexture(JsonObject json, String label) {
        rejectUnknown(json, Set.of("type", "location"), label);
    ...
    private List<GuiDefinition.TextOverlay> parseTexts(JsonObject root, String label) {
        ...
        Set<String> fields = Set.of("text", "translate", "x", "y", "color", "shadow", "layer", "anchor");
        for (int index = 0; index < array.size(); index++) {
    ...
    private List<GuiDefinition.TextRule> parseTextRules(JsonObject root, String label) {
        ...
        for (int index = 0; index < array.size(); index++) {
            ...
            JsonObject json = object(array.get(index), itemLabel);
            rejectUnknown(json, Set.of("selector", "x_offset", "y_offset", "color", "hidden"), itemLabel);
    ...
    private static void checkDepth(JsonElement value, int depth, String label) {
        ...
        if (value.isJsonArray()) {
            ...
            for (JsonElement child : value.getAsJsonObject().asMap().values()) checkDepth(child, depth + 1, label);
        }
    }
    ...
    public record ParsedDefinition(
            String id,
            DefinitionCandidate.Operation operation,
            int priority,
            MatchSpec matcher,
