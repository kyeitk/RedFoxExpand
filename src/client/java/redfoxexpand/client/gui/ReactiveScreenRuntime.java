...
final class ReactiveScreenRuntime {
    ...
    void initialize(RuntimeSnapshot snapshot) {
        initialize(snapshot, 0L);
    }

    void initialize(RuntimeSnapshot snapshot, long nowMillis) {
        previous = snapshot;
        current = snapshot;
        if (snapshot != null) {
    ...
    void tick(RuntimeSnapshot snapshot, long nowMillis) {
        if (snapshot == null) {
            previous = null;
            current = null;
            animations.clear();
