...
本文是 RedFoxExpand 1.7.10 `0.2.1` 所实现的 Schema v3.1 独立协议文档。它完整描述资源发现、Definition、匹配器、几何与 Slot、Scene Graph、纹理与文字、Runtime Context、表达式、Binding、Event、Behavior、Action、属性动画、限制、错误与回退；阅读本文不需要先阅读 v2 或 v3.0 文档。

Schema v3.1 已在 Minecraft 1.7.10 项目的 `0.2.1` 中实现；本文只描述该目标的真实能力和限制。
...
Minecraft 1.7.10 示例必须声明资源包格式 `1`：
...
Manifest 和其引用的所有 config 必须使用完全相同的 `api_version`。1.7.10 `0.2.1` 在加载 config 时执行精确版本校验；`3` 与 `3.1` 不相等，混用会拒绝该 config。
...
> 1.7.10 平台可用 matcher 为 `all/any/not`、screen/menu 完整类名或简单类名的
...
`menu_type` 与 ResourceLocation 必须为小写 namespaced ID，例如 `minecraft:player`。在 1.7.10 中，这三个现代 matcher 不可用并在 reload 拒绝，不会静默返回 false。纹理对象的 `resource_location` 类型仍然可用，与同名 matcher 无关。
...
- 窗口 Resize 或第三方 GUI 改变原版容器位置时，实时 GUI origin 会继续更新；1.7.10 原版没有 Recipe Book。
...
`location` 最长 512，必须使用 `/`，禁止绝对路径、盘符、空段、`.`、`..`。`resource_location` 和 `gui_sprite` 必须是合法小写 namespaced ID。Minecraft 1.7.10 没有现代 GUI atlas，因此 `gui_sprite` 兼容映射为 `textures/gui/sprites/<path>.png`。
...
前 15 项对应 Runtime Context 数据；`PROPERTY_*` 对应可绑定/动画的属性能力；`EVENT_*` 与 `ACTION_*` 对应 Behavior 能力。当前 1.7.10 平台实现全部上列 Reactive 能力。材质包只能通过使用相应变量、Property、Event 或 Action 间接要求能力，不能直接声明 Capability。
...
Geometry 用于整体逻辑 GUI 扩展；SlotModifier 用于指定 Slot 细调；Element 使用 GUI Anchor。三者都会以实时 GUI origin 为基础，因此 Resize 或第三方 GUI 重定位后仍可重新定位。若 Element 使用 Screen Anchor，则它有意独立于 GUI Slot 移动。1.7.10 原版没有 Recipe Book。
...
9. F3+T 后检查关闭重开、Resize、Tooltip、Slot 与变换复位；Recipe Book 在 1.7.10 不适用。
...
- 本文协议已在 1.7.10 `0.2.1` 实现；跨版本资源包仍须使用目标版本支持的 matcher 与 `pack_format`；
...
- 真实 Minecraft GUI、F3+T、Resize、Tooltip 遮挡及其他 Mod 兼容会受客户端环境影响；Recipe Book 在 1.7.10 不适用。
