...
本文是 RedFoxExpand 26.2 `0.2.1` 所实现的 Schema v3.1 独立协议文档。它完整描述资源发现、Definition、匹配器、几何与 Slot、Scene Graph、纹理与文字、Runtime Context、表达式、Binding、Event、Behavior、Action、属性动画、限制、错误与回退；阅读本文不需要先阅读 v2 或 v3.0 文档。

Schema v3.1 只在 Minecraft 26.2 项目的 `0.2.1` 中实现。RedFoxExpand 1.8.9 与 1.7.10 的 `0.2.0` 当前只实现到 Schema v3.0。
...
- “实现行为”：由当前解析器与运行时定义。
- “平台表现”：可能受 Minecraft、渲染后端与其他 Mod 组合影响。
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---:|---|
| `api_version` | JSON number | 是 | 必须写数值 `3.1`，不能写字符串 `"3.1"` 或整数 `31` |
...
规范要求 Manifest 和其引用的所有 config 使用相同的 `api_version: 3.1`。当前 `0.2.1` 的 v3/v3.1 共用解析器，源码未对两处的 `3` 与 `3.1` 组合实施单独的“精确相等”检查；这是校验缺口，不是可依赖的兼容能力。材质包作者必须保持两处完全一致。
...
`menu_type` 与 ResourceLocation 必须为小写 namespaced ID，例如 `minecraft:player`。`resource_location`/`mod_namespace` 依赖 GUI 纹理捕获，可能在屏幕刚创建、第一次背景绘制前尚不可用；匹配结果会在捕获后重新求解。
...
- Recipe Book 和窗口 Resize 改变原版容器位置时，实时 GUI origin 会继续更新。
...
`location` 最长 512，必须使用 `/`，禁止绝对路径、盘符、空段、`.`、`..`。`resource_location` 和 `gui_sprite` 必须是合法小写 namespaced ID。
...
前 15 项对应 Runtime Context 数据；`PROPERTY_*` 对应可绑定/动画的属性能力；`EVENT_*` 与 `ACTION_*` 对应 Behavior 能力。当前 26.2 平台实现全部上列能力。材质包只能通过使用相应变量、Property、Event 或 Action 间接要求能力，不能直接声明 Capability。
...
| 变量 | 类型 | 说明 |
|---|---|---|
| `event.old_health` | number | 变化前生命 |
| `event.new_health` | number | 变化后生命 |
...
| `player.burning.started` | `is_burning` false -> true | 无 | 否 |
| `player.burning.stopped` | `is_burning` true -> false | 无 | 否 |
...
F3+T 构建新的不可变 ResourceSnapshot。只有完整完成发现、解析、场景校验、表达式编译和纹理预算校验后才发布新 generation。
...
每个 Scene Sprite 绘制使用成对矩阵 push/pop，变换不会有意泄漏到后续原版绘制。与其他渲染 Mod 组合时的表现可能因环境而异。
...
Geometry 用于整体逻辑 GUI 扩展；SlotModifier 用于指定 Slot 细调；Element 使用 GUI Anchor。三者都会以实时 GUI origin 为基础，因此 Recipe Book 和 Resize 后仍可重新定位。若 Element 使用 Screen Anchor，则它有意独立于 GUI Slot 移动。
...
9. F3+T 后检查关闭重开、Recipe Book、Resize、Tooltip、Slot 与变换复位。
...
- Schema v3.1 只在 26.2 `0.2.1` 实现；1.8.9/1.7.10 尚未支持；
...
- Manifest/config 的 3 与 3.1 精确相等校验存在前述实现缺口，作者必须自行保持一致；
- Minecraft GUI、F3+T、Recipe Book、Resize、Tooltip 遮挡、其他 Mod 兼容与渲染状态恢复可能因环境组合而异。
...
Event:
  ...
  player.burning.started
  player.burning.stopped
  screen.opened
