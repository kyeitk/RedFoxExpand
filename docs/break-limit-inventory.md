...
本文记录 RedFoxExpand 1.8.9 对“超出原版背包贴图边界”的实现约定，供后续开发和资源包作者使用。
...
明确的渲染阶段表达前后关系，不直接依赖大幅负 Z 值。绘制期间会暂时关闭 scissor 和深度测试，完成后
...
- 重载时先在局部构建并验证新 immutable generation，成功后原子替换并刷新当前容器；顶层失败保留旧 generation。
...
1. 将模板复制到 `assets/Kyeitk/config/`，图片放到 `assets/Kyeitk/textures/gui/`。
2. 放入与 `tex_width/tex_height` 完全一致的 PNG。
3. 打开生存背包，确认背景完整显示且原版槽位仍与贴图对齐。
4. 背包保持打开至少两个客户端 tick，确认原版背景、槽位、玩家模型和文字不会在下一瞬间左移。
5. 分别在无药水效果和有可显示药水效果时打开背包，确认 GUI、槽位和贴图位置不变，药水列表显示在
   GUI 右侧；缩小窗口后确认列表贴近右边界而不会推动 GUI。
6. 检查新增区域是否可见，并在不同 GUI 缩放和窗口尺寸下观察裁切情况。
7. 保持背包打开执行资源重载，确认尺寸和槽位没有累计漂移。
8. 禁用资源包并重载，确认 GUI 恢复原始尺寸。

游戏内验证由用户执行；开发侧只保证加载、匹配、几何和渲染调用链闭环。
