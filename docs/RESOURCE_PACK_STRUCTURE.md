...
- v2/v3/v3.1 的相对资源从小写 `kyeitk` domain 读取；
- manifest config 必须从声明它的同一资源包读取；1.7.10 通过持有实际 `IResourcePack` 保证此语义；
- 当前 1.7.10 `0.2.1` 与 1.8.9 分支都读取同一 v2/v3/v3.1 结构；两个版本都使用 `pack_format: 1`。涉及版本类 matcher 时，
...
该入口只保证文档列出的兼容字段。新包优先使用 Kyeitk v1 或 native v2/v3/v3.1。
...
坏 JSON、非法路径、缺失纹理或超预算内容只跳过对应来源。一个坏 v1/v2/v3/v3.1 文件不会全局关闭 legacy 或其他
有效配置。
