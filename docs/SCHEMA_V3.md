...
1.8.9 平台层在 Forge `ClientTickEvent.END` 每 tick 提供：
...
- 系统时钟调整不会使动画倒退；1.8.9 使用 `System.nanoTime()` 单调时钟。
...
热路径不进行文件/ZIP/JSON/PNG/Expression parse、反射或资源路径解析。1.8.9 renderer 仅在现有
...
1.8.9 没有现代注册表化菜单/GUI 资源标识，因此 `menu_type`、`resource_location`、`mod_namespace`
matcher 会在重载时明确拒绝；请使用 exact/assignable screen/menu class、simple class 或 title matcher。
`net.minecraft.world.inventory.InventoryMenu` 会兼容映射到 1.8.9 的
