...
1.7.10 `0.2.0` 同时接受 v1、v2 与 v3。一个 manifest 的 `api_version` 决定它引用的全部 config 版本；manifest v3
...
1.7.10 平台层在 Forge `ClientTickEvent.END` 每 tick 提供：
...
`gui.x/y` 使用当前 `GuiContainer` 完成 geometry 后的实时原点；窗口 resize/重新 `initGui` 后会从新原点
建立 runtime。`mouse.gui_x = mouse.x - gui.x`，`mouse.gui_y = mouse.y - gui.y`。
...
若 1.7.10 的 LWJGL cursor/window 缩放输入因零尺寸窗口产生非有限值，平台 adapter 会把对应
...
Binding 求值后合成一次，用于启动循环动画。1.7.10 没有 Recipe Book；窗口 resize/`initGui`、F3+T、
候选集合变化、LocalPlayer 更换或关闭后重开都会重建 runtime，因此会再次触发 `screen.opened`。
...
- resize/重新 initGui：重建 runtime，清除动画和 accumulator，再以新 geometry 触发一次 `screen.opened`；
...
- 系统时钟调整不会使动画倒退；1.7.10 使用 `System.nanoTime()` 单调时钟。
...
热路径不进行文件/ZIP/JSON/PNG/Expression parse、反射或资源路径解析。1.7.10 renderer 仅在现有
`AlphaBlendState`/quad 桥内调用 LWJGL，并恢复颜色、混合、alpha、depth、texture、scissor 与矩阵状态。
...
1.7.10 没有 Recipe Book，也没有现代注册表化菜单/GUI 资源标识，因此 `menu_type`、`resource_location`、`mod_namespace`
matcher 会在重载时明确拒绝；请使用 exact/assignable screen/menu class、simple class 或 title matcher。
`net.minecraft.world.inventory.InventoryMenu` 会兼容映射到 1.7.10 的
`net.minecraft.inventory.ContainerPlayer`。这不会改变 Schema v3 Reactive Core 的语义。
