...
0.1.0 保留一条不依赖外部文件的玩家背包契约测试。测试通过正式 `GuiConfigLoader` 解析两张
Kyeitk 相对路径贴图，并验证 `screen_center`、`underlay`、浮点尺寸和完整纹理取样语义。

测试 JSON 与小型 RGBA PNG 在运行时生成，不把本地参考材质包、第三方图片或其 ZIP 纳入公开仓库。
因此全新克隆可独立构建，同时不会改变或扩大任何第三方素材授权。
