...
1. 启用自己的最小测试资源包并打开生存背包。
2. 确认测试 PNG 出现在目标位置，透明背景正常。
3. 修改 `x/y` 后执行 F3+T，确认位置实时刷新且不累计漂移。
4. 修改 `width/height`，确认整张图片按比例缩放，没有只显示左上角。
5. 切换 `underlay/background/foreground`，确认层级符合预期。
6. 建立一条 `region` 配置，确认 `image_x/image_y` 能改变原图取样区域。

上述游戏内视觉检查由用户执行。项目的 `gradlew build` 会自动检查配置解析、锚点换算、UV 换算、
非法区域配置和玩家背包类名兼容，不会启动 Minecraft。
